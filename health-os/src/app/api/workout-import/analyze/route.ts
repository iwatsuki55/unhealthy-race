...
export async function POST(request: Request) {
  try {
    ...
    const draft = await extractWorkoutDraftWithOpenAI(images);

    return NextResponse.json({
      draft,
      warnings: [
        "This draft was extracted from screenshots. Review all fields before saving.",
        "Low-confidence or missing fields should be corrected manually."
      ]
    });
  } catch (error) {
    if (error instanceof Error && error.message === "WORKOUT_IMPORT_OPENAI_NOT_CONFIGURED") {
      return NextResponse.json(
        {
          error:
            "Workout analysis is not configured yet. Set OPENAI_API_KEY on the server and try again."
        },
        { status: 503 }
      );
    }
    ...
    return NextResponse.json(
      {
        error:
          "Health OS could not analyze these screenshots. The images are still available so you can retry."
      },
      { status: 500 }
    );
  }
